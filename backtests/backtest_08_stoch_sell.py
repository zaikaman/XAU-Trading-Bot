"""
Backtest SMC + Stochastic + Sell Filter
=========================================
Base: SMC-Only v4 (100% synced with main_live.py)
Added:
  1. Stochastic Filter: BUY blocked if K > 75, SELL blocked if K < 25
  2. Sell Filter Strict: SELL requires ML agree + conf >= 55%

Combined logic for SELL:
  - Blocked if Stoch K < 25 (oversold)
  - Blocked if ML signal != SELL (no ML agreement)
  - Blocked if ML confidence < 55%
BUY:
  - Blocked if Stoch K > 75 (overbought)

Exit: ALL 3 systems unchanged (SmartPositionManager + SmartRiskManager + Time/Trend)

Usage:
    python backtests/backtest_stoch_sell.py
"""
...
# ─── SMC + Stochastic + Sell Filter Backtest ─────────────────

class SMCStochSellBacktest:
    """SMC-Only v4 + Stochastic Filter + Sell Filter Strict. All exit systems unchanged."""
    ...
    def run(self, df, start_date=None, end_date=None, initial_capital=5000.0):
        ...
        print(f"\n  Running SMC + Stochastic + Sell Filter backtest...")
...
def generate_xlsx_report(stats: BacktestStats, filepath: str, start_date, end_date):
    ...
    ws["A1"] = "XAUBot AI — SMC + Stochastic + Sell Filter Backtest"
...
def generate_log(stats: BacktestStats, filepath: str, start_date, end_date):
    ...
    lines = []
    lines.append("=" * 80)
    lines.append("XAUBOT AI — SMC + Stochastic + Sell Filter Backtest Log")
    lines.append("=" * 80)
    lines.append(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append(f"Period: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
    lines.append(f"Strategy: SMC-Only v4 + Stochastic (K={STOCH_K_PERIOD}) + Sell Filter (ML >= {SELL_FILTER_MIN_ML_CONF:.0%})")
    lines.append("")
...
def main():
    print("=" * 70)
    print("XAUBOT AI — SMC + Stochastic + Sell Filter Backtest")
    ...
    print("\n" + "=" * 70)
    print("SMC + STOCHASTIC + SELL FILTER — RESULTS")
    print("=" * 70)
    ...
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "08_stoch_sell_results")
    os.makedirs(output_dir, exist_ok=True)

    log_path = os.path.join(output_dir, f"stoch_sell_{timestamp}.log")
    xlsx_path = os.path.join(output_dir, f"stoch_sell_{timestamp}.xlsx")
