"""
Backtest SMC + EMA Stack Filter
=================================
Base: SMC-Only v4 (100% synced with main_live.py)
Added: EMA 50 Trend Filter + EMA Stack Score

EMA Stack Filter Logic:
  - Calculate EMA 50 on M15 (EMA 9/21 already exist from FeatureEngineer)
  - BUY blocked if close < EMA 50 (bearish macro trend)
  - SELL blocked if close > EMA 50 (bullish macro trend)
  - Track EMA stack alignment: 9 > 21 > 50 (bullish) or 9 < 21 < 50 (bearish)

Exit: ALL 3 systems unchanged (SmartPositionManager + SmartRiskManager + Time/Trend)

Usage:
    python backtests/backtest_ema_stack.py
"""
...
class SimulatedTrade:
    ...
    ema_21: float = 0.0
    ema_50: float = 0.0
    ema_stack: str = "none"  # "bullish", "bearish", "partial", "none"
...
class BacktestStats:
    ...
    ema_filtered: int = 0
    ema_filtered_buy_below_50: int = 0
    ema_filtered_sell_above_50: int = 0
    # EMA Stack alignment tracking
    trades_with_full_stack: int = 0
    trades_against_stack: int = 0


# ─── EMA 50 Calculation ──────────────────────────────────────

def calculate_ema_50(df: pl.DataFrame) -> pl.DataFrame:
    """Calculate EMA 50 on close price. EMA 9 and 21 already exist from FeatureEngineer."""
    df = df.with_columns([
...
def get_ema_stack(ema_9: float, ema_21: float, ema_50: float) -> str:
...
# ─── SMC + EMA Stack Backtest ────────────────────────────────

class SMCEMAStackBacktest:
    """SMC-Only v4 + EMA 50 Trend Filter. All exit systems unchanged."""
    ...
    def run(self, df, start_date=None, end_date=None, initial_capital=5000.0):
        ...
        ema_9_list = df["ema_9"].to_list()
        ema_21_list = df["ema_21"].to_list()
        ema_50_list = df["ema_50"].to_list()
        ...
        print(f"\n  Running SMC + EMA Stack backtest...")
        print(f"  EMA Filter: BUY blocked if close < EMA50, SELL blocked if close > EMA50")
        print(f"  EMA Stack tracked: 9 > 21 > 50 (bullish) | 9 < 21 < 50 (bearish)")
        ...
        for i in range(start_idx, end_idx):
            ...
            # ═══════════════════════════════════════════════════════
            # EMA STACK FILTER — the ONLY addition to baseline
            # ═══════════════════════════════════════════════════════
            current_close = closes_list[i]
            current_ema_9 = ema_9_list[i] if i < len(ema_9_list) else current_close
            current_ema_21 = ema_21_list[i] if i < len(ema_21_list) else current_close
            current_ema_50 = ema_50_list[i] if i < len(ema_50_list) else current_close

            # Core filter: close vs EMA 50
            if smc_signal.signal_type == "BUY":
                if current_close < current_ema_50:
                    stats.ema_filtered += 1
                    stats.ema_filtered_buy_below_50 += 1
                    continue

            if smc_signal.signal_type == "SELL":
                if current_close > current_ema_50:
                    stats.ema_filtered += 1
                    stats.ema_filtered_sell_above_50 += 1
                    continue

            # Track EMA stack alignment
            ema_stack = get_ema_stack(current_ema_9, current_ema_21, current_ema_50)

            is_full_stack = (
                (smc_signal.signal_type == "BUY" and ema_stack == "bullish") or
                (smc_signal.signal_type == "SELL" and ema_stack == "bearish")
            )
            is_against_stack = (
                (smc_signal.signal_type == "BUY" and ema_stack == "bearish") or
                (smc_signal.signal_type == "SELL" and ema_stack == "bullish")
            )
            ...
            trade = SimulatedTrade(
                ...
                ema_9=current_ema_9, ema_21=current_ema_21, ema_50=current_ema_50,
                ema_stack=ema_stack,
            )
            stats.trades.append(trade)

            # Track stack alignment
            if is_full_stack:
                stats.trades_with_full_stack += 1
            if is_against_stack:
                stats.trades_against_stack += 1
...
def generate_xlsx_report(stats: BacktestStats, filepath: str, start_date, end_date):
    ...
    ws["A1"] = "XAUBot AI — SMC + EMA Stack Filter Backtest"
    ...
    summary_data = [
        ...
        ("EMA Filtered (Total)", stats.ema_filtered, False),
        ("  BUY blocked (< EMA50)", stats.ema_filtered_buy_below_50, False),
        ("  SELL blocked (> EMA50)", stats.ema_filtered_sell_above_50, False),
        ("Trades w/ Full Stack", stats.trades_with_full_stack, False),
        ("Trades Against Stack", stats.trades_against_stack, False),
    ...
    # EMA Stack Analysis
    row += 1
    ws.cell(row=row, column=4, value="EMA Stack Alignment")
    ws.cell(row=row, column=4).font = subheader_font
    ws.cell(row=row, column=4).fill = subheader_fill
    for c in range(5, 8):
        ws.cell(row=row, column=c).fill = subheader_fill
    row += 1
    for lbl, col in [("Stack", 4), ("Trades", 5), ("WR", 6), ("Net PnL", 7)]:
        ws.cell(row=row, column=col, value=lbl).font = Font(bold=True)
    row += 1
    for stack_type in ["bullish", "bearish", "partial"]:
        st = [t for t in stats.trades if t.ema_stack == stack_type]
        sw = sum(1 for t in st if t.result == TradeResult.WIN)
        sp = sum(t.profit_usd for t in st)
        swr = sw / len(st) * 100 if st else 0
        ws.cell(row=row, column=4, value=stack_type.title())
        ws.cell(row=row, column=5, value=len(st))
        ws.cell(row=row, column=6, value=f"{swr:.1f}%")
        ws.cell(row=row, column=7, value=f"${sp:,.2f}")
        ws.cell(row=row, column=7).font = Font(color="006100" if sp >= 0 else "9C0006")
        row += 1
    ...
    headers = [
        ...
        "Mode", "EMA 9", "EMA 21", "EMA 50", "Stack",
    ]
    ...
    for ri, t in enumerate(stats.trades, 2):
        vals = [
            ...
            round(t.ema_9, 2), round(t.ema_21, 2), round(t.ema_50, 2), t.ema_stack,
        ]
...
def generate_log(stats: BacktestStats, filepath: str, start_date, end_date):
    ...
    lines = []
    lines.append("=" * 80)
    lines.append("XAUBOT AI — SMC + EMA Stack Filter Backtest Log")
    lines.append("=" * 80)
    lines.append(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append(f"Period: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
    lines.append(f"Strategy: SMC-Only v4 + EMA 50 Trend Filter + Stack Tracking")
    lines.append("")
    lines.append("--- EMA STACK FILTER STATS ---")
    lines.append(f"  Total Blocked:            {stats.ema_filtered}")
    lines.append(f"  BUY blocked (< EMA50):    {stats.ema_filtered_buy_below_50}")
    lines.append(f"  SELL blocked (> EMA50):   {stats.ema_filtered_sell_above_50}")
    lines.append(f"  Trades w/ Full Stack:     {stats.trades_with_full_stack}")
    lines.append(f"  Trades Against Stack:     {stats.trades_against_stack}")
    lines.append("")
    ...
    lines.append("--- EMA STACK ALIGNMENT ---")
    for stack_type in ["bullish", "bearish", "partial"]:
        st = [t for t in stats.trades if t.ema_stack == stack_type]
        sw = sum(1 for t in st if t.result == TradeResult.WIN)
        sp = sum(t.profit_usd for t in st)
        swr = sw / len(st) * 100 if st else 0
        lines.append(f"  {stack_type:10s}: {len(st):3d} trades, {swr:5.1f}% WR, ${sp:>8,.2f}")
    lines.append("")
    ...
    lines.append(f"{'#':>4} {'Entry Time':>16} {'Dir':>4} {'Entry':>10} {'Exit':>10} {'P/L($)':>8} {'Result':>6} {'Exit Reason':>18} {'Stack':>8} {'Session':>20}")
    lines.append("-" * 150)
    for idx, t in enumerate(stats.trades, 1):
        lines.append(
            ...
            f"{t.result.value:>6} {t.exit_reason.value:>18} {t.ema_stack:>8} "
            f"{t.session:>20}"
        )
    lines.append("\n" + "=" * 80)
...
def main():
    print("=" * 70)
    print("XAUBOT AI — SMC + EMA Stack Filter Backtest")
    print("Base: SMC-Only v4 (100% synced)")
    print("Added: EMA 50 Trend Filter (BUY if close>EMA50, SELL if close<EMA50)")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("SMC + EMA STACK FILTER — RESULTS")
    print("=" * 70)

    print(f"\n  EMA Filter Stats:")
    print(f"    Total blocked:       {stats.ema_filtered}")
    print(f"    BUY below EMA50:     {stats.ema_filtered_buy_below_50}")
    print(f"    SELL above EMA50:    {stats.ema_filtered_sell_above_50}")
    print(f"    Full stack trades:   {stats.trades_with_full_stack}")
    print(f"    Against stack:       {stats.trades_against_stack}")
    ...
    for stack_type in ["bullish", "bearish", "partial"]:
        ...
        sp = sum(t.profit_usd for t in st)
        swr = sw / len(st) * 100 if st else 0
        print(f"    {stack_type:10s}: {len(st)} trades, {swr:.1f}% WR, ${sp:,.2f}")
    ...
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "07_ema_stack_results")
    os.makedirs(output_dir, exist_ok=True)

    log_path = os.path.join(output_dir, f"ema_stack_{timestamp}.log")
    xlsx_path = os.path.join(output_dir, f"ema_stack_{timestamp}.xlsx")
