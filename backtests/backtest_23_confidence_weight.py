"""
Backtest #23 — Confidence Weight Rebalance
============================================
Base: SMC-Only v4 (Backtest #1)
Modification: Rebalance SMC confidence weights to prioritize FVG/OB over BOS

Current weights:
  base:               0.40
  structure_aligned:  0.15
  bos_choch:          0.12
  fvg:                0.08
  ob:                 0.10
  trend_strength:     0.10
...
Hypothesis: FVG and OB are more reliable reversal signals than BOS.
Rebalancing should improve lot sizing decisions and potentially filter quality.

Configs:
  A: Boost FVG+OB    (fvg=0.14, ob=0.14, bos=0.06)
  B: FVG dominant     (fvg=0.18, ob=0.10, bos=0.06)
  C: OB dominant      (ob=0.18, fvg=0.10, bos=0.06)
  D: Require FVG|OB   (entry filter: must have FVG or OB to enter)
  E: High min conf    (require confidence >= 0.55 to enter)

Usage:
    python backtests/backtest_23_confidence_weight.py
"""
...
# ─── Confidence Weight Backtest ───────────────────────────────

class ConfidenceWeightBacktest:
    """SMC-Only + custom confidence weights / entry filters."""
    ...
    def __init__(
        ...
        # Confidence weight overrides
        w_bos: float = 0.12,
        w_fvg: float = 0.08,
        w_ob: float = 0.10,
        # Entry filter
        require_fvg_or_ob: bool = False,
        min_confidence: float = 0.0,  # 0 = no min filter
    ):
    ...
    def _recalculate_confidence(
        ...
        """Recalculate confidence with custom weights."""
        conf = 0.40  # base
        ...
        # Trend strength
        if df_slice is not None and "bos" in df_slice.columns:
            recent_bos = df_slice.tail(20)["bos"].to_list()
            if signal_type == "BUY":
    ...
    # ── Full exit simulation (synced with baseline #1) ──
    ...
    def run(self, df, start_date=None, end_date=None, initial_capital=5000.0):
        ...
        print(f"  Weights: bos={self.w_bos}, fvg={self.w_fvg}, ob={self.w_ob}")
        print(f"  Require FVG|OB: {self.require_fvg_or_ob}, Min conf: {self.min_confidence}")
        ...
        for i in range(start_idx, end_idx):
            ...
            # ═══ #23: ENTRY FILTER — Require FVG or OB ═══
            if self.require_fvg_or_ob and not has_fvg and not has_ob:
                stats.filtered_signals += 1
                continue

            # Market structure for confidence calc
            market_structure = 0
            if "bos" in df_slice.columns:
                recent_bos_vals = df_slice.tail(20)["bos"].to_list()
                bull_bos = sum(1 for b in recent_bos_vals if b == 1)
                bear_bos = sum(1 for b in recent_bos_vals if b == -1)
            ...
            # ═══ #23: CUSTOM CONFIDENCE CALCULATION ═══
            confidence = self._recalculate_confidence(
...
def main():
    print("=" * 70)
    print("XAUBOT AI — #23 Confidence Weight Rebalance")
    print("Base: SMC-Only v4 | Modified: Prioritize FVG/OB over BOS")
    print("=" * 70)
    ...
    configs = [
        # name, w_bos, w_fvg, w_ob, require_fvg_ob, min_conf
        ("A: boost_fvg_ob",     0.06, 0.14, 0.14, False, 0.0),
        ("B: fvg_dominant",     0.06, 0.18, 0.10, False, 0.0),
        ("C: ob_dominant",      0.06, 0.10, 0.18, False, 0.0),
        ("D: require_fvg|ob",   0.12, 0.08, 0.10, True,  0.0),
        ("E: min_conf_0.55",    0.12, 0.08, 0.10, False, 0.55),
    ]

    all_results = []

    for cfg_name, w_bos, w_fvg, w_ob, req_fvg_ob, min_conf in configs:
        print(f"\n{'=' * 60}")
        print(f"  Config: {cfg_name}")

        bt = ConfidenceWeightBacktest(
            w_bos=w_bos,
            w_fvg=w_fvg,
            w_ob=w_ob,
            require_fvg_or_ob=req_fvg_ob,
    ...
    # ═══ SUMMARY ═══
    print(f"\n{'=' * 70}")
    print("#23 CONFIDENCE WEIGHT — ALL CONFIGURATIONS")
    print("=" * 70)

    print(f"\n  {'Config':<22} {'Trades':>6} {'WR':>6} {'Net PnL':>10} {'DD':>6} {'Sharpe':>7} {'PF':>5} {'Filt':>5} {'vs Base':>10}")
    print(f"  {'-' * 85}")
    print(f"  {'BASELINE (#1)':<22} {'686':>6} {'72.2%':>6} {'$1,449.86':>10} {'5.4%':>6} {'1.98':>7} {'1.52':>5} {'—':>5} {'—':>10}")
    print(f"  {'#8 Stoch+Sell':<22} {'416':>6} {'76.7%':>6} {'$1,320.41':>10} {'2.8%':>6} {'3.17':>7} {'1.76':>5} {'—':>5} {'—':>10}")

    best_pnl = -999999
    best_name = ""
    best_stats = None
    for cfg_name, stats, net_pnl, diff in all_results:
        print(f"  {cfg_name:<22} {stats.total_trades:>6} {stats.win_rate:>5.1f}% ${net_pnl:>9,.2f} {stats.max_drawdown:>5.1f}% {stats.sharpe_ratio:>7.2f} {stats.profit_factor:>5.2f} {stats.filtered_signals:>5} ${diff:>+9,.2f}")
        if net_pnl > best_pnl:
            best_pnl = net_pnl
            best_name = cfg_name
            best_stats = stats
    ...
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "23_confidence_weight_results")
    os.makedirs(output_dir, exist_ok=True)

    log_path = os.path.join(output_dir, f"conf_weight_{timestamp}.log")
    with open(log_path, "w") as f:
        f.write(f"#23 Confidence Weight Rebalance Results\n")
        f.write(f"Generated: {datetime.now()}\n\n")
        for cfg_name, stats, net_pnl, diff in all_results:
            f.write(f"  {cfg_name}: {stats.total_trades} trades, {stats.win_rate:.1f}% WR, ${net_pnl:,.2f}, filtered: {stats.filtered_signals}, vs base: ${diff:+,.2f}\n")
        f.write(f"\nBest: {best_name}\n")
    print(f"  Log saved: {log_path}")

    try:
        from backtests.backtest_01_smc_only import generate_xlsx_report as gen_xlsx
        xlsx_path = os.path.join(output_dir, f"conf_weight_{timestamp}.xlsx")
