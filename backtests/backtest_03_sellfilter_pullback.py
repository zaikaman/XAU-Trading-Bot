"""
Backtest SMC-Only + Sell Filter Strict + Pullback Filter
=========================================================
...
IMPROVEMENT #3 — Pullback Filter (re-enable from main_live.py):
  ...
  - Rationale: Prevents entry during whipsaw/bounces that cause early losses

Usage:
    python backtests/backtest_sellfilter_pullback.py
"""
...
class BacktestStats:
    ...
    sell_filtered: int = 0      # SELL blocked by sell filter
    pullback_filtered: int = 0  # Blocked by pullback filter


# ─── SMC-Only + Sell Filter + Pullback Filter ────────────────

class SMCOnlySellPullback:
    """
    Base: 100% synced with main_live.py Signal Logic v4 + all exit systems.
    ADDED: Sell Filter Strict + Pullback Filter (both re-enabled).
    """

    SELL_FILTER_MIN_ML_CONF = 0.55  # ML must be >= 55% for SELL
    ...
    def __init__(
        ...
        try:
            ...
            print("  [WARN] ML model not loaded — exit ML checks & sell filter disabled")
    ...
    # ══════════════════════════════════════════════════════════════
    # IMPROVEMENT #3: Pullback Filter (synced from main_live.py)
    # ══════════════════════════════════════════════════════════════
    ...
    def _check_pullback_filter(
        ...
        # Pullback detection logic (synced)
    ...
    def _simulate_trade_exit(
        ...
        """
        Simulate trade exit — UNCHANGED from baseline (same as backtest_smc_only.py).
        All improvements are on ENTRY side (sell filter + pullback filter).
        """
        pip_value = 10
        ...
        for i in range(entry_idx + 1, min(entry_idx + max_bars, len(df))):
            ...
            # ════════════════════════════════════════════════
            # B) SmartRiskManager checks (UNCHANGED — same as baseline)
            # ════════════════════════════════════════════════
            ...
            # B.3 Early cut — UNCHANGED (baseline: 30% loss, momentum < -30)
            if current_profit < 0:
    ...
    def run(
        ...
        print(f"\n  Running SMC-Only + Sell Filter + Pullback Filter backtest...")
        ...
        for i in range(start_idx, end_idx):
            ...
            # ══════════════════════════════════════════════════════════
            # IMPROVEMENT #2: SELL FILTER STRICT
            # ══════════════════════════════════════════════════════════
            if smc_signal.signal_type == "SELL":
                # Require ML to agree for SELL signals
                if ml_signal != "SELL":
            ...
            # ══════════════════════════════════════════════════════════
            # IMPROVEMENT #3: PULLBACK FILTER
            # ══════════════════════════════════════════════════════════
            current_price = df_slice.tail(1)["close"].item()
            can_trade_pullback, pullback_reason = self._check_pullback_filter(
...
def generate_xlsx_report(stats: BacktestStats, filepath: str, start_date: datetime, end_date: datetime):
    ...
    ws["A1"] = "XAUBot AI — Sell Filter + Pullback Filter Backtest"
    ...
    ws["A4"] = f"Changes: Sell Filter Strict (ML agree + 55%) + Pullback Filter (momentum/MACD/EMA9)"
    ...
    summary_data = [
        ...
        ("Sell Filtered", stats.sell_filtered, False),
    ...
    # Exit Reason + Session + SMC (same layout as baseline)
    exit_counts = {}
    for t in stats.trades:
        exit_counts[t.exit_reason.value] = exit_counts.get(t.exit_reason.value, 0) + 1

    ws.cell(row=6, column=4, value="Exit Reasons").font = subheader_font
    ...
    row += 1
    ws.cell(row=row, column=4, value="Session Performance").font = subheader_font
    ...
    if len(stats.equity_curve) > 1:
        chart = LineChart()
        chart.title = "Equity Curve (Sell Filter + Pullback)"
        chart.style = 10
    ...
    ws5["A1"] = "Comparison: Sell Filter + Pullback vs Baseline"
    ws5["A1"].font = Font(name="Calibri", bold=True, size=14, color="1F4E79")
    ws5["A3"] = "Metric"
    ws5["B3"] = "Baseline (no filter)"
    ws5["C3"] = "Sell+Pullback"
    ws5["D3"] = "Delta"
    for c in range(1, 5):
    ...
    baseline = {
        "Total Trades": 686, "Wins": 495, "Losses": 191,
        "Win Rate": 72.2, "Net PnL": 1449.86, "Profit Factor": 1.42,
        "Max Drawdown %": 5.4, "Avg Win": 9.92, "Avg Loss": 18.11,
        "Expectancy": 2.11, "Sharpe Ratio": 1.98,
        "Early Cut Count": 92,
    }
    improved = {
        "Total Trades": stats.total_trades, "Wins": stats.wins, "Losses": stats.losses,
        "Win Rate": stats.win_rate, "Net PnL": net_pnl, "Profit Factor": stats.profit_factor,
        "Max Drawdown %": stats.max_drawdown, "Avg Win": stats.avg_win, "Avg Loss": stats.avg_loss,
        "Expectancy": stats.expectancy, "Sharpe Ratio": stats.sharpe_ratio,
...
def generate_log(stats: BacktestStats, filepath: str, start_date: datetime, end_date: datetime):
    ...
    lines = []
    lines.append("=" * 80)
    lines.append("XAUBOT AI — Sell Filter + Pullback Filter Backtest Log")
    lines.append("=" * 80)
    lines.append(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append(f"Period: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
    lines.append(f"Strategy: SMC-Only v4 + Sell Filter Strict + Pullback Filter")
    lines.append("")
    lines.append("--- IMPROVEMENTS APPLIED ---")
    lines.append(f"  #2 Sell Filter: SELL requires ML agree + conf >= {SMCOnlySellPullback.SELL_FILTER_MIN_ML_CONF:.0%}")
    lines.append(f"  #3 Pullback Filter: Block entry during counter-momentum (ATR-based)")
    lines.append(f"  Sell signals blocked:     {stats.sell_filtered}")
    lines.append(f"  Pullback signals blocked: {stats.pullback_filtered}")
    lines.append("")
    ...
    lines.append(f"  Baseline Net PnL:  $1,449.86  |  Improved: ${net_pnl:,.2f}  |  Delta: ${net_pnl - 1449.86:+,.2f}")
    lines.append(f"  Baseline WR: 72.2%  |  Improved: {stats.win_rate:.1f}%")
    lines.append(f"  Baseline Trades: 686  |  Improved: {stats.total_trades}")
    ec_count = sum(1 for t in stats.trades if t.exit_reason == ExitReason.EARLY_CUT)
    lines.append(f"  Baseline Early Cut: 92  |  Improved: {ec_count}")
    lines.append("")
...
def main():
    print("=" * 70)
    print("XAUBOT AI — Sell Filter + Pullback Filter Backtest")
    print("Base: SMC-Only v4 (100% synced)")
    print("Added: #2 Sell Filter Strict + #3 Pullback Filter")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("SELL FILTER + PULLBACK FILTER BACKTEST RESULTS")
    print("=" * 70)

    print(f"\n  Filters Applied:")
    print(f"    Sell Filter:     {stats.sell_filtered} SELL signals blocked")
    print(f"    Pullback Filter: {stats.pullback_filtered} signals blocked")
    ...
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "03_sellfilter_pullback_results")
    os.makedirs(output_dir, exist_ok=True)

    log_path = os.path.join(output_dir, f"sellfilter_pullback_{timestamp}.log")
    xlsx_path = os.path.join(output_dir, f"sellfilter_pullback_{timestamp}.xlsx")
