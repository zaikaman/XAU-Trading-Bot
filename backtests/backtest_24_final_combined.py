"""
Backtest #24 — Final Combined (All 3 Winners)
===============================================
Base: SMC-Only v4 (Backtest #1)
Combined improvements:
  #19B: Skip Tokyo-London overlap session (+$345 vs baseline)
  #20B: Early cut momentum < -50 instead of -30 (+$125 vs baseline)
  #22C: ATR-Adaptive wider exit (BE=3.0x, trail_start=5.0x, trail_step=3.0x) (+$395 vs baseline)

Also test #22D variant (tight BE + wide trail):
  Config A: #19B + #20B + #22C (wider)
  Config B: #19B + #20B + #22D (tight BE + wide trail)

Usage:
    python backtests/backtest_24_final_combined.py
"""
...
class FinalCombinedBacktest:
    """SMC-Only + #19B (skip TL) + #20B (early cut mom<-50) + #22C/D (ATR-adaptive exit)."""
    ...
    def __init__(
        ...
        # #20B: Early cut tuning
        early_cut_momentum: float = -50.0,
        early_cut_loss_pct: float = 30.0,
        # #22: ATR-adaptive exit
        be_mult: float = 3.0,
        trail_start_mult: float = 5.0,
        ...
        self._ticket_counter = 2240000

    # ── Session filter (#19B: skip Tokyo-London) ──
    ...
    # ── Full exit simulation (#20B early cut + #22 ATR-adaptive) ──
    ...
    def run(self, df, start_date=None, end_date=None, initial_capital=5000.0):
        ...
        print(f"  Skip Tokyo-London: {self.skip_tokyo_london}")
        print(f"  Early cut: mom<{self.early_cut_momentum}, loss>={self.early_cut_loss_pct}%")
        print(f"  ATR-adaptive: BE={self.be_mult}x, trail_start={self.trail_start_mult}x, trail_step={self.trail_step_mult}x")
...
def main():
    print("=" * 70)
    print("XAUBOT AI — #24 Final Combined (All 3 Winners)")
    print("#19B Skip TL + #20B Early Cut mom<-50 + #22 ATR-Adaptive")
    print("=" * 70)
    ...
    # ═══ CONFIG A: #19B + #20B + #22C (wider) ═══
    configs = [
        ("A: 19B+20B+22C (wider)",      3.0, 5.0, 3.0),
        ("B: 19B+20B+22D (tight+wide)",  2.0, 4.0, 3.0),
    ]

    all_results = []

    for cfg_name, be_m, ts_m, step_m in configs:
        print(f"\n{'=' * 60}")
        print(f"  Config: {cfg_name}")

        bt = FinalCombinedBacktest(
            skip_tokyo_london=True,
            early_cut_momentum=-50.0,
            early_cut_loss_pct=30.0,
            be_mult=be_m,
            trail_start_mult=ts_m,
            trail_step_mult=step_m,
        )
        ...
        print(f"    Session blocked: {stats.session_blocked}")
    ...
    # ═══ FINAL SUMMARY ═══
    print(f"\n{'=' * 70}")
    print("#24 FINAL COMBINED — ALL CONFIGURATIONS")
    print("=" * 70)

    print(f"\n  {'Config':<30} {'Trades':>6} {'WR':>6} {'Net PnL':>10} {'DD':>6} {'Sharpe':>7} {'PF':>5} {'vs Base':>10}")
    print(f"  {'-' * 90}")
    print(f"  {'BASELINE (#1)':<30} {'686':>6} {'72.2%':>6} {'$1,449.86':>10} {'5.4%':>6} {'1.98':>7} {'1.52':>5} {'—':>10}")
    print(f"  {'#8 Stoch+Sell':<30} {'416':>6} {'76.7%':>6} {'$1,320.41':>10} {'2.8%':>6} {'3.17':>7} {'1.76':>5} {'—':>10}")
    print(f"  {'#19B Skip TL':<30} {'683':>6} {'73.4%':>6} {'$1,794.94':>10} {'5.2%':>6} {'2.41':>7} {'1.54':>5} {'$+345.08':>10}")
    print(f"  {'#20B EC mom<-50':<30} {'681':>6} {'73.4%':>6} {'$1,575.33':>10} {'5.4%':>6} {'2.12':>7} {'1.46':>5} {'$+125.47':>10}")
    print(f"  {'#21 Combined 19B+20B':<30} {'679':>6} {'74.4%':>6} {'$1,857.58':>10} {'5.3%':>6} {'2.46':>7} {'1.56':>5} {'$+407.72':>10}")
    print(f"  {'#22C ATR wider':<30} {'732':>6} {'76.1%':>6} {'$1,845.24':>10} {'4.4%':>6} {'2.33':>7} {'1.55':>5} {'$+395.38':>10}")
    print(f"  {'#22D ATR tight+wide':<30} {'743':>6} {'79.1%':>6} {'$1,822.86':>10} {'4.3%':>6} {'2.35':>7} {'1.58':>5} {'$+373.00':>10}")

    best_pnl = -999999
    best_name = ""
    best_stats = None
    for cfg_name, stats, net_pnl, diff in all_results:
        print(f"  {cfg_name:<30} {stats.total_trades:>6} {stats.win_rate:>5.1f}% ${net_pnl:>9,.2f} {stats.max_drawdown:>5.1f}% {stats.sharpe_ratio:>7.2f} {stats.profit_factor:>5.2f} ${diff:>+9,.2f}")
        if net_pnl > best_pnl:
            best_pnl = net_pnl
            best_name = cfg_name
            best_stats = stats
    ...
    for sess, d in sorted(session_stats.items(), key=lambda x: -x[1]["p"]):
        ...
        print(f"    {sess:<30}: {total:>3} trades, {wr:>5.1f}% WR, ${d['p']:>8,.2f}")
    print(f"    Session blocked (TL skip): {best_stats.session_blocked}")
    ...
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "24_final_combined_results")
    os.makedirs(output_dir, exist_ok=True)

    log_path = os.path.join(output_dir, f"final_combined_{timestamp}.log")
    with open(log_path, "w") as f:
        f.write(f"#24 Final Combined Results (All 3 Winners)\n")
    ...
    try:
        ...
        xlsx_path = os.path.join(output_dir, f"final_combined_{timestamp}.xlsx")
