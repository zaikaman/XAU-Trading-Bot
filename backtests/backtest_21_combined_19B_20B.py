...
Hypothesis: Both improvements are independent (entry filter vs exit tuning),
so they should stack additively or even synergistically.

Usage:
    python backtests/backtest_21_combined_19B_20B.py
"""
...
class CombinedBacktest:
    """SMC-Only + Skip Tokyo-London (#19B) + Early Cut mom<-50 (#20B)."""
    ...
    # ── Full exit simulation (synced with #1, #20B early cut tuned) ──
...
def main():
    print("=" * 70)
    print("XAUBOT AI — #21 Combined #19B + #20B")
    print("Skip Tokyo-London + Early Cut mom<-50")
    print("=" * 70)
    ...
    print(f"\n{'=' * 70}")
    print("#21 COMBINED RESULTS — #19B + #20B")
    print("=" * 70)
    ...
    print(f"  {'#19B Skip TL (alone)':<30} {'683':>6} {'73.4%':>6} {'$1,794.94':>10} {'5.2%':>6} {'2.41':>7} {'1.54':>5} {'$+345.08':>10}")
    print(f"  {'#20B EC mom<-50 (alone)':<30} {'681':>6} {'73.4%':>6} {'$1,575.33':>10} {'5.4%':>6} {'2.12':>7} {'1.46':>5} {'$+125.47':>10}")
    diff = net_pnl - baseline_pnl
    print(f"  {'#21 COMBINED':<30} {stats.total_trades:>6} {stats.win_rate:>5.1f}% ${net_pnl:>9,.2f} {stats.max_drawdown:>5.1f}% {stats.sharpe_ratio:>7.2f} {stats.profit_factor:>5.2f} ${diff:>+9,.2f}")
    ...
    print(f"    Session blocked (TL skip): {stats.session_blocked}")
    ...
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "21_combined_results")
    os.makedirs(output_dir, exist_ok=True)

    log_path = os.path.join(output_dir, f"combined_{timestamp}.log")
    with open(log_path, "w") as f:
        f.write(f"#21 Combined #19B + #20B Results\n")
    ...
    try:
        ...
        xlsx_path = os.path.join(output_dir, f"combined_{timestamp}.xlsx")
        gen_xlsx(stats, xlsx_path, start_date, end_date)
