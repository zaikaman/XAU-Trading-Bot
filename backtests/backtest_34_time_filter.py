"""
Backtest #34 -- Time-of-Hour + Day-of-Week Filter
===================================================
Base: #31B (H1 Price vs EMA20) -- 625 trades, 81.8% WR, $2,807, Sharpe 3.97

Idea: Analyze which WIB hours and weekdays have worst WR, then skip them.
Two-phase approach:
  Phase 1: Run baseline, collect per-hour and per-day performance stats
  Phase 2: Run filtered configs that skip the worst hours/days

Configs (determined by Phase 1 analysis):
  A: Skip worst 2 WIB hours
  B: Skip worst 3 WIB hours
  C: Skip worst weekday(s)
  D: Skip worst 2 hours + worst weekday
  E: Skip worst 3 hours + worst weekday

Usage:
    python backtests/backtest_34_time_filter.py
"""
...
class TimeFilterBacktest:
    """#31B base + time-of-hour and day-of-week filtering."""
    ...
    def run(self, df_m15, df_h1, start_date=None, end_date=None, initial_capital=5000.0):
        ...
        print(f"  #34 skip hours(WIB): [{skip_hours_str}], skip days: [{skip_days_str}]")
...
def main():
    print("=" * 70)
    print("XAUBOT AI -- #34 Time-of-Hour + Day-of-Week Filter")
    print("Base: #31B (H1 Price vs EMA20) | Modified: Skip worst hours/days")
    print("=" * 70)
    ...
    # ===============================================================
    print(f"\n{'=' * 60}")
    print("  PHASE 1: Baseline analysis (no time filter)")
    bt_base = TimeFilterBacktest()
    ...
    print(f"\n  Baseline: {stats_base.total_trades} trades, {stats_base.win_rate:.1f}% WR, ${net_base:,.2f}")
    ...
    worst_2_days = set(d[0] for d in day_ranking[:2]) if len(day_ranking) >= 2 else worst_day
    ...
    # ===============================================================
    print(f"\n{'=' * 60}")
    print("  PHASE 2: Testing filtered configurations")
    ...
    for cfg_name, cfg_params in configs:
        ...
        bt = TimeFilterBacktest(**cfg_params)
    ...
    # === FINAL SUMMARY ===
    print(f"\n{'=' * 70}")
    print("#34 TIME-OF-HOUR + DAY-OF-WEEK FILTER -- ALL CONFIGURATIONS")
    print("=" * 70)

    print(f"\n  {'Config':<25} {'Trades':>6} {'WR':>6} {'Net PnL':>10} {'DD':>6} {'Sharpe':>7} {'PF':>5} {'Blocked':>8} {'vs #31B':>10}")
    print(f"  {'-' * 90}")
    print(f"  {'#24B (base)':<25} {'739':>6} {'80.4%':>6} {'$2,235':>10} {'3.4%':>6} {'2.87':>7} {'1.77':>5} {'--':>8} {'--':>10}")
    print(f"  {'#28B (smart BE)':<25} {'741':>6} {'79.8%':>6} {'$2,464':>10} {'3.5%':>6} {'3.23':>7} {'1.83':>5} {'--':>8} {'--':>10}")
    print(f"  {'#31B (H1 filter)':<25} {'625':>6} {'81.8%':>6} {'$2,807':>10} {'2.5%':>6} {'3.97':>7} {'2.19':>5} {'--':>8} {'--':>10}")
    ...
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "34_time_filter_results")
    os.makedirs(output_dir, exist_ok=True)

    log_path = os.path.join(output_dir, f"time_filter_{timestamp}.log")
    with open(log_path, "w") as f:
        f.write(f"#34 Time-of-Hour + Day-of-Week Filter Results\n")
    ...
    try:
        ...
        xlsx_path = os.path.join(output_dir, f"time_filter_{timestamp}.xlsx")
