"""
Backtest #22 — ATR-Adaptive Exit
=================================
Base: SMC-Only v4 (Backtest #1)
Modification: Dynamic breakeven/trail based on ATR at entry

Current fixed values:
  breakeven_pips = 30 (~ATR*2.5 in pips)
  trail_start_pips = 50 (~ATR*4.2 in pips)
  trail_step_pips = 30 (~ATR*2.5 in pips)

ATR-adaptive: scale these by actual ATR at entry time.
High volatility → wider stops, Low volatility → tighter stops.

Configs:
  A: Baseline-equivalent (BE=ATR*2.5, start=ATR*4.2, step=ATR*2.5)
  B: Tighter (BE=ATR*2.0, start=ATR*3.5, step=ATR*2.0)
  C: Wider (BE=ATR*3.0, start=ATR*5.0, step=ATR*3.0)
  D: Tight BE + wide trail (BE=ATR*2.0, start=ATR*4.0, step=ATR*3.0)
  E: Very tight (BE=ATR*1.5, start=ATR*3.0, step=ATR*1.5)

Usage:
    python backtests/backtest_22_atr_adaptive_exit.py
"""
...
# ─── ATR-Adaptive Exit Backtest ───────────────────────────────

class ATRAdaptiveBacktest:
    """SMC-Only + ATR-adaptive breakeven/trailing stop."""
    ...
    def __init__(
        ...
        # ATR-adaptive multipliers (pips = ATR * mult)
        be_mult: float = 2.5,
        trail_start_mult: float = 4.2,
        trail_step_mult: float = 2.5,
    ):
    ...
    # ── Full exit simulation (ATR-ADAPTIVE breakeven/trail) ──
    ...
    def _simulate_trade_exit(
        ...
        # Convert ATR to pips (ATR is in price, 1 pip = $0.1)
        ...
        for i in range(entry_idx + 1, min(entry_idx + max_bars, len(df))):
            ...
            if pip_profit_from_entry >= adaptive_trail_start_pips:
                trail_distance = adaptive_trail_step_pips * 0.1  # convert pips to price
                if direction == "BUY":
            ...
            # B.3 Early cut (baseline: mom<-30, loss>=30%)
            if current_profit < 0:
...
def main():
    print("=" * 70)
    print("XAUBOT AI — #22 ATR-Adaptive Exit")
    print("Base: SMC-Only v4 | Modified: Dynamic breakeven/trail from ATR")
    print("=" * 70)
    ...
    configs = [
        # name, be_mult, trail_start_mult, trail_step_mult
        ("A: baseline_equiv", 2.5, 4.2, 2.5),
        ("B: tighter", 2.0, 3.5, 2.0),
        ("C: wider", 3.0, 5.0, 3.0),
        ("D: tight_be+wide_trail", 2.0, 4.0, 3.0),
        ("E: very_tight", 1.5, 3.0, 1.5),
    ]
    ...
    # ═══ SUMMARY ═══
    print(f"\n{'=' * 70}")
    print("#22 ATR-ADAPTIVE EXIT — ALL CONFIGURATIONS")
    print("=" * 70)
    ...
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "22_atr_adaptive_results")
    os.makedirs(output_dir, exist_ok=True)

    log_path = os.path.join(output_dir, f"atr_adaptive_{timestamp}.log")
    with open(log_path, "w") as f:
        f.write(f"#22 ATR-Adaptive Exit Results\n")
        f.write(f"Generated: {datetime.now()}\n\n")
        for cfg_name, stats, net_pnl, diff in all_results:
            f.write(f"  {cfg_name}: {stats.total_trades} trades, {stats.win_rate:.1f}% WR, ${net_pnl:,.2f}, vs base: ${diff:+,.2f}\n")
        f.write(f"\nBest: {best_name}\n")
    print(f"  Log saved: {log_path}")

    try:
        from backtests.backtest_01_smc_only import generate_xlsx_report as gen_xlsx
        xlsx_path = os.path.join(output_dir, f"atr_adaptive_{timestamp}.xlsx")
