"""
Backtest SMC-Only + Early Cut IMPROVED
======================================
Base: backtest_smc_only.py (100% synced with main_live.py v4)

IMPROVEMENT #1 — Reduce early_cut aggressiveness:
  - loss threshold: 30% → 45% of max_loss_per_trade
  - momentum threshold: -30 → -50 (require stronger confirmation)
  - minimum trade duration: 2 bars (30 min M15) before early cut allowed

Rationale: Early cut was responsible for 92 losses (-$1,999 = 58% of total loss).
Many trades were cut at -$15 that could have recovered due to Gold whipsaw.

Usage:
    python backtests/backtest_earlycut_improved.py
"""
...
# ─── SMC-Only + Early Cut Improved ──────────────────────────

class SMCOnlyEarlyCutImproved:
    """
    Base: 100% synced with main_live.py Signal Logic v4 + all exit systems.
    IMPROVEMENT: Early cut threshold loosened from 30%→45%, momentum -30→-50,
    + minimum 2 bars before early cut allowed.
    """

    # ══════════════════════════════════════════════════════════════
    # IMPROVEMENT PARAMETERS (what changed vs baseline)
    # ══════════════════════════════════════════════════════════════
    EARLY_CUT_LOSS_THRESHOLD = 45       # was 30 — % of max_loss_per_trade
    EARLY_CUT_MOMENTUM_THRESHOLD = -50  # was -30 — require stronger confirmation
    EARLY_CUT_MIN_BARS = 2              # was 0  — minimum 30 min before early cut
    ...
    # ── Full exit simulation (all 3 systems + IMPROVED EARLY CUT) ──
    ...
    def _simulate_trade_exit(
        ...
        IMPROVEMENT: Early cut threshold loosened (section B.3).
        """
        pip_value = 10
        ...
        for i in range(entry_idx + 1, min(entry_idx + max_bars, len(df))):
            ...
            # A.3 Peak profit drawdown protection
            ...
            # B.2 Smart Early Exit ($5-15 + reversal)
            ...
            # ══════════════════════════════════════════════════════════
            # B.3 IMPROVED EARLY CUT (THIS IS THE CHANGE)
            # ══════════════════════════════════════════════════════════
            # BASELINE: loss_percent >= 30 AND momentum < -30 AND no min bars
            # IMPROVED: loss_percent >= 45 AND momentum < -50 AND bars >= 2
            # ══════════════════════════════════════════════════════════
            if current_profit < 0:
                loss_percent_of_max = abs(current_profit) / self.max_loss_per_trade * 100
                if (momentum < self.EARLY_CUT_MOMENTUM_THRESHOLD
                        and loss_percent_of_max >= self.EARLY_CUT_LOSS_THRESHOLD
                        and bars_since_entry >= self.EARLY_CUT_MIN_BARS):
    ...
    def run(
        ...
        print(f"\n  Running SMC-Only + Early Cut IMPROVED backtest...")
        print(f"  IMPROVEMENT: early_cut loss {self.EARLY_CUT_LOSS_THRESHOLD}% (was 30%), "
              f"momentum {self.EARLY_CUT_MOMENTUM_THRESHOLD} (was -30), "
              f"min bars {self.EARLY_CUT_MIN_BARS} (was 0)")
...
def generate_xlsx_report(stats: BacktestStats, filepath: str, start_date: datetime, end_date: datetime):
    ...
    ws["A1"] = "XAUBot AI — Early Cut IMPROVED Backtest Report"
    ...
    ws["A4"] = f"Change: early_cut loss {SMCOnlyEarlyCutImproved.EARLY_CUT_LOSS_THRESHOLD}% (was 30%), momentum {SMCOnlyEarlyCutImproved.EARLY_CUT_MOMENTUM_THRESHOLD} (was -30), min bars {SMCOnlyEarlyCutImproved.EARLY_CUT_MIN_BARS}"
    ...
    ws.cell(row=6, column=4, value="Exit Reasons")
    ws.cell(row=6, column=4).font = subheader_font
    ws.cell(row=6, column=4).fill = subheader_fill
    ws.cell(row=6, column=5).fill = subheader_fill
    ws.cell(row=6, column=6).fill = subheader_fill
    row = 7
    ...
    if len(stats.equity_curve) > 1:
        chart = LineChart()
        chart.title = "Equity Curve (Early Cut Improved)"
        chart.style = 10
    ...
    # ═══ SHEET 5: COMPARISON vs BASELINE ═══
    ws5 = wb.create_sheet("vs Baseline")
    ws5.sheet_properties.tabColor = "FF6600"
    ws5["A1"] = "Comparison: Early Cut IMPROVED vs Baseline"
    ws5["A1"].font = Font(name="Calibri", bold=True, size=14, color="1F4E79")
    ws5["A3"] = "Metric"
    ws5["B3"] = "Baseline (30%/-30)"
    ws5["C3"] = "Improved (45%/-50)"
    ws5["D3"] = "Delta"
    for c in range(1, 5):
        ws5.cell(row=3, column=c).font = subheader_font
        ws5.cell(row=3, column=c).fill = subheader_fill

    # Baseline values from previous backtest
    baseline = {
        "Total Trades": 686,
        "Wins": 495,
        "Losses": 191,
    ...
    improved = {
        ...
        "Early Cut Count": sum(1 for t in stats.trades if t.exit_reason == ExitReason.EARLY_CUT),
    }
    ...
    for metric in baseline:
        ...
        ival = improved[metric]
        ws5.cell(row=row, column=2, value=bval)
        ws5.cell(row=row, column=3, value=ival)
        if isinstance(bval, (int, float)) and isinstance(ival, (int, float)):
            delta = ival - bval
            ws5.cell(row=row, column=4, value=round(delta, 2))
            # Green if improvement, red if worse (depends on metric)
            is_better = delta > 0
            if metric in ["Losses", "Max Drawdown %", "Avg Loss", "Early Cut Count"]:
                is_better = delta < 0  # Lower is better
            ws5.cell(row=row, column=4).font = Font(bold=True, color="006100" if is_better else "9C0006")
        row += 1
...
def generate_log(stats: BacktestStats, filepath: str, start_date: datetime, end_date: datetime):
    ...
    lines = []
    lines.append("=" * 80)
    lines.append("XAUBOT AI — Early Cut IMPROVED Backtest Log")
    lines.append("=" * 80)
    ...
    lines.append(f"")
    lines.append(f"--- IMPROVEMENT APPLIED ---")
    lines.append(f"  Early Cut Loss Threshold:     {SMCOnlyEarlyCutImproved.EARLY_CUT_LOSS_THRESHOLD}% (baseline: 30%)")
    lines.append(f"  Early Cut Momentum Threshold: {SMCOnlyEarlyCutImproved.EARLY_CUT_MOMENTUM_THRESHOLD} (baseline: -30)")
    lines.append(f"  Early Cut Min Bars:           {SMCOnlyEarlyCutImproved.EARLY_CUT_MIN_BARS} bars / {SMCOnlyEarlyCutImproved.EARLY_CUT_MIN_BARS * 15} min (baseline: 0)")
    lines.append("")
    ...
    lines.append(f"  Baseline Net PnL:  ${baseline_net:,.2f}")
    lines.append(f"  Improved Net PnL:  ${net_pnl:,.2f}")
    lines.append(f"  Delta:             ${net_pnl - baseline_net:+,.2f}")
    lines.append(f"  Baseline Early Cut: 92 trades")
    ec_count = sum(1 for t in stats.trades if t.exit_reason == ExitReason.EARLY_CUT)
    lines.append(f"  Improved Early Cut: {ec_count} trades")
    lines.append(f"  Early Cut Reduced:  {92 - ec_count} fewer trades")
    lines.append("")
...
def main():
    print("=" * 70)
    print("XAUBOT AI — Early Cut IMPROVED Backtest")
    print("Base: SMC-Only v4 (100% synced) + Early Cut improvement")
    print(f"Change: loss {SMCOnlyEarlyCutImproved.EARLY_CUT_LOSS_THRESHOLD}% (was 30%), "
          f"momentum {SMCOnlyEarlyCutImproved.EARLY_CUT_MOMENTUM_THRESHOLD} (was -30), "
          f"min bars {SMCOnlyEarlyCutImproved.EARLY_CUT_MIN_BARS}")
    print("=" * 70)
    ...
    backtest = SMCOnlyEarlyCutImproved(
        capital=5000.0,
    ...
    print("\n" + "=" * 70)
    print("EARLY CUT IMPROVED BACKTEST RESULTS")
    print("=" * 70)
    print(f"\n  Strategy:  SMC-Only v4 + Early Cut IMPROVED")
    print(f"  Change:    loss {SMCOnlyEarlyCutImproved.EARLY_CUT_LOSS_THRESHOLD}% (was 30%), "
          f"momentum {SMCOnlyEarlyCutImproved.EARLY_CUT_MOMENTUM_THRESHOLD} (was -30), "
          f"min bars {SMCOnlyEarlyCutImproved.EARLY_CUT_MIN_BARS}")
    ...
    print(f"    Baseline Early Cut: 92")
    print(f"    Improved Early Cut: {ec_count}")
    ...
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "02_earlycut_improved_results")
    os.makedirs(output_dir, exist_ok=True)

    log_path = os.path.join(output_dir, f"earlycut_improved_{timestamp}.log")
    xlsx_path = os.path.join(output_dir, f"earlycut_improved_{timestamp}.xlsx")
