"""
Backtest SMC-Only + Sell Filter Strict ONLY
=============================================
Base: backtest_smc_only.py (100% synced with main_live.py v4)

IMPROVEMENT #2 — Sell Filter Strict (re-enable from backtest_live_sync.py):
  - SELL signals require ML agreement (ml_signal == "SELL")
  - SELL signals require ML confidence >= 55%
  - BUY signals: UNCHANGED — no additional filter
  - NO pullback filter

Rationale: SELL WR=68.8% vs BUY WR=74.6%, SELL profit $162 vs BUY $1,287.
Filter bad SELL trades while keeping all BUY trades intact.

Usage:
    python backtests/backtest_sellfilter_only.py
"""
...
class SMCOnlySellFilterOnly:
    """
    Base: 100% synced with main_live.py Signal Logic v4 + all exit systems.
    ADDED: Sell Filter Strict only (no pullback filter).
    BUY trades are completely unchanged from baseline.
    """
    ...
    def __init__(self, capital=5000.0, max_daily_loss_percent=5.0,
        ...
        try:
            self.ml_model.load()
            print("  ML model loaded (for exit evaluation + sell filter)")
        except Exception:
    ...
    def run(self, df, start_date=None, end_date=None, initial_capital=5000.0):
        ...
        print(f"\n  Running SMC-Only + Sell Filter ONLY backtest...")
        print(f"  Sell Filter: SELL requires ML agree + conf >= {self.SELL_FILTER_MIN_ML_CONF:.0%}")
        print(f"  BUY: No additional filter (same as baseline)")
        ...
        for i in range(start_idx, end_idx):
            ...
            # ML prediction (needed for sell filter + dynamic confidence + exit)
            ml_signal = ""
            ml_confidence = 0.5
            try:
            ...
            # ══════════════════════════════════════════════════════════
            # SELL FILTER STRICT (the only improvement)
            # BUY: pass through unchanged
            # SELL: requires ML agree + confidence >= 55%
            # ══════════════════════════════════════════════════════════
            if smc_signal.signal_type == "SELL":
                if ml_signal != "SELL":
                    ...
                    continue
                if ml_confidence < self.SELL_FILTER_MIN_ML_CONF:
                    stats.sell_filtered += 1
...
def generate_xlsx_report(stats, filepath, start_date, end_date):
    ...
    ws["A1"] = "XAUBot AI — Sell Filter ONLY Backtest"
    ...
    ws["A4"] = f"Change: SELL requires ML agree + conf >= {SMCOnlySellFilterOnly.SELL_FILTER_MIN_ML_CONF:.0%} | BUY unchanged"
    ...
    data = [
        ...
        ("Sell Filtered (total)", stats.sell_filtered, False),
    ...
    for label, value, is_header in data:
        ...
        if is_header:
            ws.cell(row=row, column=1).font = sf; ws.cell(row=row, column=1).fill = sfill; ws.cell(row=row, column=2).fill = sfill
        if label == "Net PnL":
    ...
    ws.cell(row=6, column=4).fill = sfill; ws.cell(row=6, column=5).fill = sfill; ws.cell(row=6, column=6).fill = sfill
    row = 7
    for r, c in sorted(ec.items(), key=lambda x: -x[1]):
        pct = c / stats.total_trades * 100 if stats.total_trades > 0 else 0
        ws.cell(row=row, column=4, value=r); ws.cell(row=row, column=5, value=c); ws.cell(row=row, column=6, value=f"{pct:.1f}%")
        row += 1
    ...
    for s, d in sorted(ss.items(), key=lambda x: -x[1]["p"]):
        ...
        ws.cell(row=row, column=4, value=s); ws.cell(row=row, column=5, value=total)
        ws.cell(row=row, column=6, value=f"{wr:.1f}%"); ws.cell(row=row, column=7, value=f"${d['p']:,.2f}")
        ws.cell(row=row, column=7).font = Font(color="006100" if d["p"]>=0 else "9C0006")
        row += 1

    for c, w in {4:28,5:10,6:12,7:14}.items(): ws.column_dimensions[get_column_letter(c)].width = w

    # Trade Log
    ws2 = wb.create_sheet("Trade Log"); ws2.sheet_properties.tabColor = "2E75B6"
    ...
    for col, h in enumerate(headers, 1):
        cell = ws2.cell(row=1, column=col, value=h); cell.font = hf; cell.fill = hfill; cell.alignment = Alignment(horizontal="center")
    ...
    for col in range(1, len(headers)+1): ws2.column_dimensions[get_column_letter(col)].width = max(11, len(headers[col-1])+3)

    # Equity
    ws3 = wb.create_sheet("Equity Curve"); ws3.sheet_properties.tabColor = "548235"
    ...
    for idx, eq in enumerate(stats.equity_curve):
        if eq > pk: pk = eq
        ws3.cell(row=idx+2, column=1, value=idx); ws3.cell(row=idx+2, column=2, value=round(eq,2)); ws3.cell(row=idx+2, column=3, value=round(pk-eq,2))
    if len(stats.equity_curve) > 1:
        chart = LineChart(); chart.title = "Equity Curve (Sell Filter Only)"; chart.style = 10
        chart.y_axis.title = "Equity ($)"; chart.x_axis.title = "Trade #"; chart.width = 30; chart.height = 15
        d = Reference(ws3, min_col=2, min_row=1, max_row=len(stats.equity_curve)+1)
        chart.add_data(d, titles_from_data=True); chart.series[0].graphicalProperties.line.width = 20000
        ws3.add_chart(chart, "E2")

    # Daily PnL
    ws4 = wb.create_sheet("Daily PnL"); ws4.sheet_properties.tabColor = "BF8F00"
    dpnl = {}
    for t in stats.trades:
    ...
    for ri, (day, d) in enumerate(sorted(dpnl.items()), 2):
        wr = d["wins"]/d["trades"]*100 if d["trades"]>0 else 0; cum += d["profit"]
        ws4.cell(row=ri, column=1, value=day); ws4.cell(row=ri, column=2, value=d["trades"]); ws4.cell(row=ri, column=3, value=d["wins"])
        ws4.cell(row=ri, column=4, value=f"{wr:.0f}%"); ws4.cell(row=ri, column=5, value=round(d["profit"],2)); ws4.cell(row=ri, column=6, value=round(cum,2))
    ...
    # vs Baseline
    ws5 = wb.create_sheet("vs Baseline"); ws5.sheet_properties.tabColor = "FF6600"
    ws5["A1"] = "Comparison: Sell Filter Only vs Baseline"; ws5["A1"].font = Font(name="Calibri", bold=True, size=14, color="1F4E79")
    ws5["A3"] = "Metric"; ws5["B3"] = "Baseline"; ws5["C3"] = "Sell Filter Only"; ws5["D3"] = "Delta"
    for c in range(1,5): ws5.cell(row=3, column=c).font = sf; ws5.cell(row=3, column=c).fill = sfill
    baseline = {"Total Trades":686,"Wins":495,"Losses":191,"Win Rate":72.2,"Net PnL":1449.86,"Profit Factor":1.42,"Max Drawdown %":5.4,"Avg Win":9.92,"Avg Loss":18.11,"Expectancy":2.11,"Sharpe Ratio":1.98}
    improved = {"Total Trades":stats.total_trades,"Wins":stats.wins,"Losses":stats.losses,"Win Rate":stats.win_rate,"Net PnL":net_pnl,"Profit Factor":stats.profit_factor,"Max Drawdown %":stats.max_drawdown,"Avg Win":stats.avg_win,"Avg Loss":stats.avg_loss,"Expectancy":stats.expectancy,"Sharpe Ratio":stats.sharpe_ratio}
    row = 4
    for m in baseline:
        ws5.cell(row=row, column=1, value=m); ws5.cell(row=row, column=2, value=baseline[m]); ws5.cell(row=row, column=3, value=improved[m])
        if isinstance(baseline[m],(int,float)) and isinstance(improved[m],(int,float)):
            delta = improved[m] - baseline[m]; ws5.cell(row=row, column=4, value=round(delta,2))
            better = delta > 0
            if m in ["Losses","Max Drawdown %","Avg Loss"]: better = delta < 0
...
def generate_log(stats, filepath, start_date, end_date):
    ...
    L = []
    L.append("=" * 80)
    L.append("XAUBOT AI — Sell Filter ONLY Backtest Log")
    L.append("=" * 80)
    L.append(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    L.append(f"Period: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
    L.append(f"Strategy: SMC-Only v4 + Sell Filter Strict (no pullback)")
    L.append("")
    L.append("--- SELL FILTER STATS ---")
    L.append(f"  Total SELL blocked:   {stats.sell_filtered}")
    L.append(f"    ML disagree:        {stats.sell_filtered_no_ml_agree}")
    L.append(f"    Low ML confidence:  {stats.sell_filtered_low_conf}")
    L.append("")
    ...
    L.append(f"  Baseline: 686 trades, 72.2% WR, $1,449.86 net")
    L.append(f"  Improved: {stats.total_trades} trades, {stats.win_rate:.1f}% WR, ${net_pnl:,.2f} net")
    L.append(f"  Delta:    ${net_pnl - 1449.86:+,.2f}")
    L.append("")
...
def main():
    print("=" * 70)
    print("XAUBOT AI — Sell Filter ONLY Backtest")
    print("Base: SMC-Only v4 (100% synced)")
    print(f"Added: Sell Filter Strict (ML agree + conf >= {SMCOnlySellFilterOnly.SELL_FILTER_MIN_ML_CONF:.0%})")
    print("BUY: No change | NO pullback filter")
    print("=" * 70)
    ...
    if len(df) == 0:
        print("ERROR: No data"); mt5.disconnect(); return
    ...
    try:
        regime_detector.load(); df = regime_detector.predict(df); print("  HMM regime loaded")
    except Exception:
        print("  [WARN] HMM not available")
    print("  Indicators calculated")

    bt = SMCOnlySellFilterOnly(
    ...
    print("\n" + "=" * 70)
    print("SELL FILTER ONLY — RESULTS")
    print("=" * 70)

    print(f"\n  Sell Filter Stats:")
    print(f"    Total blocked:   {stats.sell_filtered}")
    ...
    out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "05_sellfilter_only_results")
    os.makedirs(out_dir, exist_ok=True)
    log_path = os.path.join(out_dir, f"sellfilter_only_{ts}.log")
    xlsx_path = os.path.join(out_dir, f"sellfilter_only_{ts}.xlsx")
