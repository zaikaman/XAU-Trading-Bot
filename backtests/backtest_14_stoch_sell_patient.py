"""
Backtest: SMC + Stoch + Sell + Patient Exit
==============================================
Base: SMC-Only v4 + Stochastic Filter + Sell Filter Strict
Changed: EXIT parameters relaxed — let winners run longer

Entry filters (unchanged from backtest_stoch_sell.py):
  1. Stochastic: BUY blocked if K > 75, SELL blocked if K < 25
  2. Sell Filter: SELL requires ML agree + conf >= 55%

Exit changes (same as backtest_patient_exit.py):
  - Breakeven: 80 pips, Trail start: 100 pips, Trail dist: 60 pips
  - Early cut: 50% loss + mom < -50
  - Timeout: 6h/8h/12h (instead of 4h/6h/8h)

Usage:
    python backtests/backtest_stoch_sell_patient.py
"""
...
# ─── SMC + Stoch + Sell + Patient Exit Backtest ──────────────

class StochSellPatientBacktest:
    """SMC-Only v4 + Stochastic + Sell Filter + Patient Exit — relaxed exit parameters."""
    ...
    def __init__(
        ...
        try:
            self.ml_model.load()
            print("  ML model loaded (for exit + stoch filter + sell filter)")
        except Exception:
    ...
    def _simulate_trade_exit(
        ...
        """
        PATIENT EXIT variant — relaxed exit parameters:
        BE=80, Trail=100/60, EarlyCut=50%+mom<-50, Timeout=6h/8h/12h
        """
        pip_value = 10
        highs = df["high"].to_list()
        lows = df["low"].to_list()
        ...
        for i in range(entry_idx + 1, min(entry_idx + max_bars, len(df))):
            ...
            # A.1 Breakeven move (after 80 pips — PATIENT)
            ...
            # A.2 Trailing SL (after 100 pips — PATIENT)
            ...
            # B.3 Early cut (PATIENT: 50% + mom < -50)
            if current_profit < 0:
    ...
    def run(self, df, start_date=None, end_date=None, initial_capital=5000.0):
        ...
        print(f"\n  Running SMC + Stoch + Sell + Patient Exit backtest...")
...
def generate_xlsx_report(stats: BacktestStats, filepath: str, start_date, end_date):
    ...
    ws["A1"] = "XAUBot AI — SMC + Stoch + Sell + Patient Exit Backtest"
...
def generate_log(stats: BacktestStats, filepath: str, start_date, end_date):
    ...
    lines = []
    lines.append("=" * 80)
    lines.append("XAUBOT AI — SMC + Stoch + Sell + Patient Exit Backtest Log")
    lines.append("=" * 80)
    lines.append(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append(f"Period: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
    lines.append(f"Strategy: SMC-Only v4 + Stochastic (K={STOCH_K_PERIOD}) + Sell Filter (ML >= {SELL_FILTER_MIN_ML_CONF:.0%}) + Patient Exit")
    lines.append("")
...
def main():
    print("=" * 70)
    print("XAUBOT AI — SMC + Stoch + Sell + Patient Exit Backtest")
    print("Entry: SMC-Only v4 + Stochastic + Sell Filter")
    print("Exit:  Patient (BE=80, Trail=100/60, 6h/8h/12h)")
    ...
    backtest = StochSellPatientBacktest(
        capital=5000.0,
    ...
    print("\n" + "=" * 70)
    print("SMC + STOCH + SELL + PATIENT EXIT — RESULTS")
    print("=" * 70)
    ...
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "14_stoch_sell_patient_results")
    os.makedirs(output_dir, exist_ok=True)

    log_path = os.path.join(output_dir, f"stoch_sell_patient_{timestamp}.log")
    xlsx_path = os.path.join(output_dir, f"stoch_sell_patient_{timestamp}.xlsx")
